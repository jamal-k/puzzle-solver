...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        ...
        False
        """
        # list showing whether each open position is possible to fill with
        # an available symbol
        position_results = []
        ...
        for m in range(len(self._symbols)):
            ...
            if self._symbols[m] == "*":
                # no symbol found yet to fill position m
                symbol_found = False
                for symbol in self._symbol_set:
                    if not (symbol in self._row_set(m) |
                            self._column_set(m) |
                            self._subsquare_set(m)):
                        # available symbol found for position m
                        symbol_found = True
                position_results.append(symbol_found)

        # return True if there is an open position that can't be filled,
        # False otherwise
        return False in position_results
